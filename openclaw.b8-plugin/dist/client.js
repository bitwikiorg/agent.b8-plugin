...
const WRITE_API_KEY_ENV_VARS = ["B8_USER_API_KEY", "BITHUB_USER_API_KEY"];
const BASE_URL_ENV_VARS = ["B8_BASE_URL", "BITHUB_BASE_URL", "BITHUB_URL"];
...
export class BithubClient {
    ...
    async createTopic(params, signal) {
        const body = {
    ...
    async deployCore(params, signal) {
        return this.createTopic(params, signal);
    }
    async replyToTopic(params, signal) {
        const body = {
    ...
    async sendPrivateMessage(params, signal) {
    ...
    async sendChatMessage(params, signal) {
    ...
    async watchTopic(params, signal) {
    ...
    async listAgents(registryTopicId, signal) {
    ...
    async getFirstPostRaw(topic, signal) {
    ...
    async requestJson(method, path, options = {}) {
        if (options.authRequired && !this.userApiKey) {
            throw new BithubConfigError("Write operations require a BIThub Discourse user API key. Configure plugins.entries.bithub-discourse with userApiKey or export B8_USER_API_KEY/BITHUB_USER_API_KEY.");
        }
        ...
        if (!response.ok) {
            ...
            throw new BithubRequestError(`BIThub API request failed (${response.status}) for ${path}: ${detail}`, response.status);
        }
...
export function createBithubClient(config = {}, fetchImpl) {
...
export function extractPostIds(topic) {
    const postStream = asRecord(topic.post_stream);
    const stream = Array.isArray(postStream?.stream) ? postStream.stream : [];
    const explicit = stream.filter((value) => typeof value === "number");
...
function resolveUserApiKey(value) {
...
function resolveRegistryTopicId(value) {
...
function extractMarkdownTableBlocks(raw) {
