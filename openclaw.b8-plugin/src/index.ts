...
const configSchema = Type.Object(
  {
    ...
    registryTopicId: Type.Optional(
      Type.Integer({ minimum: 1, description: "Agent registry topic id override." }),
    ),
  },
  { additionalProperties: false },
);

function client(config: BithubPluginConfig): BithubClient {
...
export default defineToolPlugin({
  ...
  tools: (tool) => [
    ...
    tool({
      ...
      parameters: Type.Object({
        registry_topic_id: Type.Optional(
          Type.Integer({ minimum: 1, description: "Optional registry topic id override." }),
        ),
      }),
      execute: ({ registry_topic_id }, config, context) =>
        client(config).listAgents(registry_topic_id, context.signal),
    }),
    tool({
      name: "b8_watch_topic",
      ...
      parameters: Type.Object({
        ...
        last_post_id: Type.Optional(
          Type.Integer({ minimum: 0, description: "Most recent known post id. Defaults to 0." }),
        ),
        timeout_seconds: Type.Optional(
          Type.Integer({ minimum: 1, description: "Overall watch timeout. Defaults to 60 seconds." }),
        ),
        poll_interval_seconds: Type.Optional(
          Type.Number({ exclusiveMinimum: 0, description: "Polling interval in seconds. Defaults to 5." }),
        ),
      }),
      execute: ({ topic_id, last_post_id, timeout_seconds, poll_interval_seconds }, config, context) =>
        client(config).watchTopic(
          {
            topicId: topic_id,
    ...
    tool({
      ...
      execute: ({ title, raw, category_id, tags }, config, context) =>
        client(config).createTopic(
          {
            title,
            raw,
    ...
    tool({
      ...
      parameters: Type.Object({
        ...
        reply_to_post_number: Type.Optional(
          Type.Integer({ minimum: 1, description: "Optional post number for nested replies." }),
        ),
      }),
      execute: ({ topic_id, raw, reply_to_post_number }, config, context) =>
        client(config).replyToTopic(
          {
            topicId: topic_id,
            raw,
    ...
    tool({
      ...
      execute: ({ recipients, title, raw }, config, context) =>
        client(config).sendPrivateMessage({ recipients, title, raw }, context.signal),
    }),
    tool({
      ...
      execute: ({ channel_id, message }, config, context) =>
        client(config).sendChatMessage({ channelId: channel_id, message }, context.signal),
    }),
  ],
});
